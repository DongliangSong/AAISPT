...
public class PerformanceDataGenerator {
    ...
    public static String path = "D:/data_";
    ...
    public static void generateActiveData() {
        ...
        for (double SNR : SNRs) {
            ...
            while (r <= 15) {
                ...
                for (int i = 0; i < N; i++) {
                    ...
                    double sigmaPosNoise = Math.sqrt(diffusioncoefficient * dt
                            + drift * drift * dt * dt) / SNR;
                    AbstractSimulator sim1 = new ActiveTransportSimulator(
                            drift, angleVelocity, dt, dimension, tracklength);
                    AbstractSimulator sim2 = new FreeDiffusionSimulator(
                            diffusioncoefficient, dt, dimension, tracklength);
            ...
            exportCSV(path + "/perform_active_SNR_" + SNR + "_N_" + N
    ...
    public static void generateSubdiffusionData() {
        ...
        for (double alpha : alphas) {
            ...
            AbstractSimulator sim1 = new AnomalousDiffusionWMSimulation(
                    diffusioncoefficient, dt, dimension, 2000, alpha);
            ...
            exportCSV(path + "/perform_subdiffusion_SNR_" + SNR + "_N_"
        ...
        for (double SNR : SNRs) {
            ...
            while (B <= 4) {
                ...
                AbstractSimulator sim1 = new ConfinedDiffusionSimulator(
                        diffusioncoefficient, dt, radius, dimension, tracklength);
                for (int i = 0; i < N; i++) {
                    Trajectory t = sim1.generateTrajectory();
                    t = SimulationUtil.addPositionNoise(t, sigmaPosNoise);
                    String[] res = wcp.windowedClassification(t, rrf, w, 1);
            ...
            exportCSV(path + "/perform_confined_SNR_" + SNR + "_N_" + N
    ...
    public static void generateNormalDiffData() {
        ...
        for (double SNR : SNRs) {
            ...
            exportCSV(path + "/perform_normal_SNR_" + SNR + "_N_" + N
                    + ".csv", "w", par, cor, sdcor, semcor);
        }
        rrf.stop();
    }

    public static void exportCSV(String path, String parameter, double[] par,
                                 double[] cor, double[] corsd, double[] semcor) {
    ...
    public String ExportResource(String resourceName) throws Exception {
        ...
        String tmpFolder;
        try {
            stream = this.getClass().getResourceAsStream(resourceName);// note
            // that each is a directory down in the "jar tree" been the jar the root of the tree
            if (stream == null) {

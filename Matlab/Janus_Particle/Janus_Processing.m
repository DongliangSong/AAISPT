...
NP1_len = 200;    % large particle size (nm)
NP2_len = 40;     % small particle size (nm)
NP_len = (NP1_len + NP2_len)/2;    % radius (nm)
pixel_size = 106; % pixel size (nm)
    ...
    for j = 1:num_file
        ...
        try
            ...
            if unique(isnan(data(:, 3)))
                ...
                % Save results
                ind = linspace(1, num_points, num_points);
                Table = array2table([ind', loc], 'VariableNames', {'Index', 'X', 'Y'});
                writetable(Table, fullfile(savepath, sprintf('%03d-%s_Single_track.xlsx', count, name)));
                count = count + 1;
                continue
            end
            ...
            % Save results
            ind = linspace(1, num_points, num_points);
            Table = array2table([ind', x1, y1, x2, y2, phi, theta], ...
                'VariableNames', {'Index','X1','Y1','X2','Y2','Azimuth','Polar'});

            writetable(Table, fullfile(savepath, sprintf('%03d-%s_results.xlsx', count, name)));
            count = count + 1;

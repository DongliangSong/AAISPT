...
if __name__ == '__main__':
    ...
    for path in paths:
        for mode in ['train', 'val', 'test']:
            traj = scipy.io.loadmat(os.path.join(path, f'{mode}.mat'))
            data, label = traj['data'], traj['label'].squeeze()

            norm_data = preprocess_tracks(data, use_xyz_diff=True)
            scipy.io.savemat(os.path.join(path, f'norm_{mode}.mat'), {'data': norm_data, 'label': label})

...
class Gen_5_clsdata():
    def gen_rotation_data(self, num_class, num_perclass, trace_len, Ns):
        ...
        return polars.reshape((num_perclass * num_class, trace_len, 1)), azimuths.reshape(
            (num_perclass * num_class, trace_len, 1))


def main_3_class(dir_name, dim):
    """
    For 2/3-D trajectory data, perform the dataset generation step.

    :param dir_name: Data file load and save paths.
    :param dim: The dimension of the translation coordinate of the trajectory.
    """
    num_class = 3
...
def main_5_class(dir_name, dim, trace_len):
    """
...
if __name__ == '__main__':
    root = r'D:\TrajSeg-Cls\TrajSEG-CLS_V3\CLS\Var_LD\5D\SNR03'
    ...
    if '2D' in root or '3D' in root:
        main_3_class(root, dim=target_dim)

    elif '4D' in root or '5D' in root:
        main_5_class(root, dim=target_dim, trace_len=301)
